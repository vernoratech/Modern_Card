      ...
      >
        ...
        >
          <img
            ...
            alt={displayName}
            loading="lazy"
            className={`rounded-lg border border-white/15 object-cover shadow-md transition-all duration-300 sm:rounded-xl ${isScrolled ? 'h-8 w-8 sm:h-10 sm:w-10' : 'h-10 w-10 sm:h-12 sm:w-12 md:h-14 md:w-14'
              }`}
          />
          <div className="flex flex-col text-left">
            <span className={`text-[9px] uppercase tracking-widest text-slate-300/70 transition-all duration-300 sm:text-[10px] ${isScrolled ? 'hidden sm:block' : 'block'
              }`}>
            ...
            <h1 className={`font-semibold leading-tight transition-all duration-300 ${isScrolled ? 'text-xs sm:text-sm md:text-base' : 'text-sm sm:text-base md:text-lg'
              }`}>
              {displayName}
            </h1>
          </div>
        ...
        <div
          className={`hidden items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-1 py-1 text-xs font-medium text-slate-100 shadow-inner shadow-white/10 transition-all duration-300 sm:text-sm md:flex ${isScrolled ? 'lg:flex' : 'md:flex'
            }`}
        >
          {navLinks.map((link) => (
            <button
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          <button
            ...
            className={`flex h-9 w-9 items-center justify-center rounded-full border border-white/15 bg-white/10 text-white transition md:hidden ${isMobileNavOpen ? 'bg-white/20' : 'hover:bg-white/20'
              }`}
            aria-label="Toggle navigation menu"
          >
            <span className="relative flex flex-col items-center justify-center gap-1.5">
              <span
                className={`block h-0.5 w-4 rounded bg-current transition-transform duration-200 ${isMobileNavOpen ? 'translate-y-1 rotate-45' : ''
                  }`}
              />
              <span
                className={`block h-0.5 w-4 rounded bg-current transition-opacity duration-200 ${isMobileNavOpen ? 'opacity-0' : 'opacity-100'
                  }`}
              />
              <span
                className={`block h-0.5 w-4 rounded bg-current transition-transform duration-200 ${isMobileNavOpen ? '-translate-y-1 -rotate-45' : ''
          ...
          <button
            ...
            className={`relative inline-flex items-center justify-center rounded-full bg-gradient-to-r from-cyan-400 via-sky-500 to-indigo-500 transition hover:from-cyan-300 hover:to-indigo-400 ${isScrolled
          ...
          >
            ...
            <span className="hidden md:inline font-semibold text-slate-900 cursor-pointer">View Cart</span>
            {itemCount > 0 && (
